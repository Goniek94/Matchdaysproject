...
import { addCollectionItem, type CollectionRarity } from "@/lib/api/collection";
import { ArrowLeft, CheckCircle, AlertCircle, Upload } from "lucide-react";
import Link from "next/link";

const RARITIES: { value: CollectionRarity; label: string; desc: string; color: string }[] = [
  { value: "common", label: "Common", desc: "Standard items", color: "border-gray-300 bg-white" },
  { value: "rare", label: "Rare", desc: "Hard to find", color: "border-blue-300 bg-blue-50" },
  { value: "epic", label: "Epic", desc: "Very special", color: "border-purple-300 bg-purple-50" },
  { value: "legendary", label: "Legendary", desc: "Truly unique", color: "border-amber-300 bg-amber-50" },
];
...
export default function AddCollectionItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success) {
        setResult("success");
        setTimeout(() => router.push("/collection/mine"), 1500);
      } else {
        setErrorMsg(res.message ?? "Failed to add item");
        setResult("error");
      }
    } catch {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="bg-white rounded-2xl p-10 text-center shadow-sm border">
          ...
          <h2 className="text-2xl font-semibold mb-2">Added to collection!</h2>
  ...
  const inputClass =
    "w-full px-4 py-3 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-black focus:border-transparent transition-all bg-white";
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-2xl mx-auto px-4 py-8 pb-20 sm:pb-8">
        ...
        <h1 className="text-3xl font-light mb-1">Add to Collection</h1>
        <p className="text-gray-500 text-sm mb-8">
          Showcase an item without selling it. You can always list it on auction later.
        </p>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                placeholder="e.g. Barcelona Home 2006/07 Ronaldinho #10"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Auction, gift, bought…"
                  className={inputClass}
                />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className={`w-10 h-6 rounded-full transition-colors flex-shrink-0 ${
                  form.isVintage ? "bg-black" : "bg-gray-200"
                }`}
              >
                <div
                  className={`w-4 h-4 bg-white rounded-full mx-1 transition-transform ${
                    form.isVintage ? "translate-x-4" : "translate-x-0"
                  }`}
                />
              </button>
